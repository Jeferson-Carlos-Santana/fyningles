const USER_NAME = document.body.dataset.username || "";
  ...
  if (lessonId === 4) {
    ...
    recognition.onresult = onResultModoNovo;
  } else {
    // MODO ANTIGO
    recognition.continuous = false;
    recognition.interimResults = false;
    recognition.onresult = onResultModoAntigo;
  }
...
// ====== MODO NOVO: CONTROLE DE SILÊNCIO (ajuste aqui) ====== xxx
const SILENCIO_FIM_FALA = 1800; // 1.8s (teste 1600 ~ 2400)
let timerSilencio = null;
let bufferTranscricao = "";

// MODO NOVO: só coleta e espera silêncio maior
async function onResultModoNovo(e) {
  ...
  // pega SEMPRE o último transcript que veio
  bufferTranscricao = (e.results && e.results[0] && e.results[0][0] && e.results[0][0].transcript)
    ? e.results[0][0].transcript
    : "";

  // reinicia timer (silêncio longo)
  if (timerSilencio) clearTimeout(timerSilencio);

  timerSilencio = setTimeout(() => {
    // se ainda estiver esperando resposta, processa
    if (!esperandoResposta) return;
    if (offlinePause) return;
    if (v !== RENDER_VERSION) return;
    if (FLAG !== 1) return;

    executarModoNovo(bufferTranscricao, v);

    // limpa buffer para não repetir
    bufferTranscricao = "";
    timerSilencio = null;
...
// ====== AQUI ESTÁ TODO O SEU CÓDIGO (igual), só que dentro desta função ======
async function executarModoNovo(textoBruto, v) {
  ...
  // trava para não executar duas vezes
  if (executarModoNovo._executando) return;
  executarModoNovo._executando = true;
  ...
  try {
    ...
    if (["next", "skip"].includes(texto)) {
      ...
      // limpa timer/buffer do modo novo
      if (timerSilencio) clearTimeout(timerSilencio);
      timerSilencio = null;
      bufferTranscricao = "";
    ...
    executarModoNovo._executando = false;
  }
}
